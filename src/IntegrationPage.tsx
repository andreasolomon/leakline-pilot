...
export default function IntegrationPage({ onWorkspace, canManage = true, manageableProviders = [], canSync = true, allowedProviders, showSandbox = true, heading }: IntegrationPageProps) {
    ...
    try {
      ...
      const payload = selected.id === 'stripe' ? { secretKey: form.secretKey }
        ...
        : selected.id === 'highlevel' ? { accessToken: form.accessToken, locationId: form.locationId }
          ...
          : selected.id === 'zoom' ? { accountId: form.accountId, clientId: form.clientId, clientSecret: form.clientSecret, meetingId: form.meetingId }
          : { apiKey: form.apiKey }
  ...
  return <section className="integrations-page">
    ...
    {selected && <div className="connection-modal-backdrop" onClick={() => setSelected(null)}><section className="connection-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setSelected(null)}><X size={18} /></button><span className="eyebrow">Connect {selected.category}</span><h2>{selected.label}</h2><p>{selected.id === 'google-calendar' ? selected.available ? 'You will be redirected to Google to grant read-only Calendar access.' : 'First add the OAuth client from Google Cloud. Use http://localhost:8797/api/integrations/google-calendar/callback as the redirect URI.' : 'Your credential is validated before it is encrypted and saved.'}</p>{error && <div className="modal-error"><AlertTriangle size={15} /><span>{error}</span></div>}
      ...
      {selected.id === 'zoom' && <><label>Zoom Account ID<input name="zoom-account-id" autoComplete="one-time-code" spellCheck={false} placeholder="Server-to-Server OAuth Account ID" value={form.accountId ?? ''} onChange={(event) => setForm({ ...form, accountId: event.target.value })} /></label><label>Zoom Client ID<input name="zoom-client-id" autoComplete="one-time-code" spellCheck={false} placeholder="Server-to-Server OAuth Client ID" value={form.clientId ?? ''} onChange={(event) => setForm({ ...form, clientId: event.target.value })} /></label><label>Zoom Client Secret<input name="zoom-client-secret" type="password" autoComplete="new-password" spellCheck={false} placeholder="Server-to-Server OAuth Client Secret" value={form.clientSecret ?? ''} onChange={(event) => setForm({ ...form, clientSecret: event.target.value })} /></label><label>Recurring coaching meeting ID<input name="zoom-meeting-id" inputMode="numeric" autoComplete="one-time-code" placeholder="123 456 7890" value={form.meetingId ?? ''} onChange={(event) => setForm({ ...form, meetingId: event.target.value })} /><small>Use the meeting number from the recurring Zoom link. Zoom attendance reports require a paid Pro plan or higher.</small></label><div className="connection-detail blocked"><strong>Required Zoom scopes</strong><span>Past meeting instances and past meeting participants, using read-only admin scopes.</span></div></>}
