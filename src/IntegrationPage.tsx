...
import { AlertTriangle, AudioLines, CalendarDays, CheckCircle2, CreditCard, Database, FlaskConical, Link2, RefreshCw, ShieldCheck, Smartphone, Unplug, Video, X } from 'lucide-react'
...
const providerIcons = { clickup: Database, quo: Smartphone, highlevel: Database, 'google-calendar': CalendarDays, stripe: CreditCard, whop: CreditCard, fanbasis: CreditCard, fathom: AudioLines, zoom: Video }
...
export default function IntegrationPage({ onWorkspace, canManage = true, manageableProviders = [], canSync = true, allowedProviders, showSandbox = true, heading }: IntegrationPageProps) {
    ...
    try {
      ...
      const payload = selected.id === 'stripe' ? { secretKey: form.secretKey }
        ...
        : selected.id === 'highlevel' ? { accessToken: form.accessToken, locationId: form.locationId }
          ...
          : selected.id === 'quo' ? { apiKey: form.apiKey, from: form.from }
          : selected.id === 'zoom' ? { accountId: form.accountId, clientId: form.clientId, clientSecret: form.clientSecret, meetingSeries: parseZoomMeetingSeries(form.meetingSeries ?? '') }
          : { apiKey: form.apiKey }
  ...
  return <section className="integrations-page">
    ...
    <article className="integration-security"><ShieldCheck size={20} /><div><strong>Built with a secure boundary</strong><span>Provider secrets never enter browser storage and synced data is encrypted on disk. Syncs read provider data; customer messages are only sent after a visible approval action. Use a dedicated or restricted credential wherever the provider offers one.</span></div></article>
    ...
    {selected && <div className="connection-modal-backdrop" onClick={() => setSelected(null)}><section className="connection-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setSelected(null)}><X size={18} /></button><span className="eyebrow">Connect {selected.category}</span><h2>{selected.label}</h2><p>{selected.id === 'google-calendar' ? selected.available ? 'You will be redirected to Google to grant read-only Calendar access.' : 'First add the OAuth client from Google Cloud. Use http://localhost:8797/api/integrations/google-calendar/callback as the redirect URI.' : 'Your credential is validated before it is encrypted and saved.'}</p>{error && <div className="modal-error"><AlertTriangle size={15} /><span>{error}</span></div>}
      ...
      {selected.id === 'quo' && <><label>Quo API key<input type="password" autoComplete="off" placeholder="Quo API key" value={form.apiKey ?? ''} onChange={(event) => setForm({ ...form, apiKey: event.target.value })} /><small>A Quo owner or admin creates this under Quo Settings → API. LeakLine validates it before saving it encrypted.</small></label><label>Sending number<input inputMode="tel" placeholder="+15551234567" value={form.from ?? ''} onChange={(event) => setForm({ ...form, from: event.target.value })} /><small>Use the Quo number in full international format. US carrier registration and Quo API messaging credits must be active.</small></label></>}
      {selected.id === 'zoom' && <><label>Zoom Account ID<input name="zoom-account-id" autoComplete="one-time-code" spellCheck={false} placeholder="Server-to-Server OAuth Account ID" value={form.accountId ?? ''} onChange={(event) => setForm({ ...form, accountId: event.target.value })} /></label><label>Zoom Client ID<input name="zoom-client-id" autoComplete="one-time-code" spellCheck={false} placeholder="Server-to-Server OAuth Client ID" value={form.clientId ?? ''} onChange={(event) => setForm({ ...form, clientId: event.target.value })} /></label><label>Zoom Client Secret<input name="zoom-client-secret" type="password" autoComplete="new-password" spellCheck={false} placeholder="Server-to-Server OAuth Client Secret" value={form.clientSecret ?? ''} onChange={(event) => setForm({ ...form, clientSecret: event.target.value })} /></label><label>Recurring coaching meetings<textarea name="zoom-meeting-series" rows={4} placeholder={'Fred coaching calls | https://zoom.us/j/1234567890\nYonas Friday call | 0987654321'} value={form.meetingSeries ?? ''} onChange={(event) => setForm({ ...form, meetingSeries: event.target.value })} /><small>Add one labelled meeting link or ID per line. A repeated room used on multiple days only needs to be added once.</small></label><div className="connection-detail blocked"><strong>Required Zoom scopes</strong><span>Past meeting instances and past meeting participants, using read-only admin scopes. Zoom attendance reports require a paid Pro plan or higher.</span></div></>}
